...
for column in ["is_unemployed", "lost_job_1mo", "job_search", "is_hired_1mo", "job_offer"]:
    ...
    print(model_path)
    onnx_path = model_path+'/onnx/'.format(column)
    print(onnx_path)
    ...
    predictions_random_df.to_csv(
        os.path.join(args.output_path, column, str(getpass.getuser())+'_random'+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv'))

    print('saved to:\n', os.path.join(args.output_path, column, str(getpass.getuser())+'_random'+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv'), 'saved')
