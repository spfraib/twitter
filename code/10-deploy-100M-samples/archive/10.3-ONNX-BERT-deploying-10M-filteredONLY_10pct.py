...
print('started 10.3-ONNX-BERT-deploying-10M-filteredONLY_10pct.py')
...
print('load filtered sample:', str(time.time() - start_time), 'seconds')
print(tweets_filtered.shape)
...
for column in ["is_unemployed", "lost_job_1mo", "job_search", "is_hired_1mo", "job_offer"]:
    ...
    predictions_filtered_df.to_csv(
        os.path.join(root_path,'pred_output_100M_ONNX_optimized', column, 'filtered'+'-'+str(SLURM_JOB_ID)+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv')
        )
    print('saved to:\n', os.path.join(root_path,'pred_output_100M_ONNX_optimized', column, 'filtered'+'-'+str(SLURM_JOB_ID)+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv'), 'saved')
