...
def get_args_from_command_line():
    ...
    parser.add_argument("--vocab_path", type=str, help="Path pickle file.", default="/scratch/da2734/twitter/jobs/running_on_200Msamples/glove_models_june23/vocab.pckl")
...
# file_to_save = os.path.join(root_path,'pred_output_100M_GLOVE_CNN_optimized', column, 'filtered'+'-'+str(SLURM_JOB_ID)+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv'
...
# tweets_filtered = tweets_filtered[:250] #DEBUG

eval_df = tweets_filtered
...
#     loop_start = time.time()
    ...
    if not os.path.exists(os.path.join(root_path,'pred_output_100M_GLOVE_CNN_optimized', column)):
        print('>>>> directory doesnt exists, creating it')
        os.makedirs(os.path.join(root_path,'pred_output_100M_GLOVE_CNN_optimized', column))   
    

    print('Save Predictions of filtered Tweets:')
    start_time = time.time()  
    
    predictions_filtered_df = pd.DataFrame(data=all_predictions_proba, columns = ['proba'])
    predictions_filtered_df = predictions_filtered_df.set_index(tweets_filtered.tweet_id)

    print(predictions_filtered_df.head())
    predictions_filtered_df.to_csv(
        os.path.join(root_path,'pred_output_100M_GLOVE_CNN_optimized', column, 'filtered'+'-'+str(SLURM_JOB_ID)+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv')
        )
    print('saved to:\n', os.path.join(root_path,'pred_output_100M_GLOVE_CNN_optimized', column, 'filtered'+'-'+str(SLURM_JOB_ID)+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv'), 'saved')
