...
for column in ["is_unemployed", "lost_job_1mo", "job_search", "is_hired_1mo", "job_offer"]:
    ...
    if not os.path.exists(os.path.join(root_path,'pred_output_1pct_sample', column)):
        print('>>>> directory doesnt exists')
        os.makedirs(os.path.join(root_path,'pred_output_1pct_sample', column))
    ...
    df_random.to_csv(
        ...
        os.path.join(root_path,'pred_output_1pct_sample', column, 'random'+'-'+str(SLURM_JOB_ID)+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv')
        )

    print(os.path.join(root_path,'pred_output_1pct_sample', column, 'random'+'-'+str(SLURM_JOB_ID)+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv'), 'saved')
