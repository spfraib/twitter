...
filtered_data_vecs_glove_mean = scale(np.concatenate([get_w2v_general(z, 200, glove_twitter,'mean') for z in tweets_filtered["text"]]))
print('time taken:', str(time.time() - start_time), 'seconds')
...
#     # In[ ]:
    ...
    data_path = "/scratch/da2734/twitter/data/may20_9Klabels/data_binary_pos_neg_balanced/preprocessed_glove/"
    print("************ {} ************".format(column))
...
#     print(type(predictions_filtered))
    ...
    df_filtered.to_csv(
        # os.path.join(root_path,'pred_output', column, 'filtered'+'-'+str(SLURM_JOB_ID)+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv')
        os.path.join(root_path,'pred_output_10pct_sample_GLOVE', column, 'filtered'+'-'+str(SLURM_JOB_ID)+'-'+str(SLURM_ARRAY_TASK_ID)+'.csv')
