...
def get_success(country_code):
    if not os.path.exists(os.path.join(path_to_timelines, this_batch, 'API', country_code, 'success')):
        ...
        success = set()
        with open(os.path.join(path_to_timelines, this_batch, 'API', country_code, 'success'), 'r',
...
def download_timelines(index_key):
    ...
    for (user_id, tweet_id) in users_block:
        ...
        if len(downloaded_ids) == cutoff or user_id == users_block[-1][0]:
            ...
            with open(os.path.join(path_to_timelines, this_batch, 'API', country_code, 'success'), 'a',
                      encoding='utf-8') as file:
...
if __name__ == '__main__':
    ...
    users = pq.ParquetDataset(
        glob(os.path.join(path_to_timelines, '*', 'most_recent_id', country_code, '*.parquet'))).read().to_pandas()

    # Keep the most recent tweets for each user
    users = users.sort_values(['user_id', 'created_at'], ascending=[True, False]).drop_duplicates('user_id',
                                                                                                  keep='first')
