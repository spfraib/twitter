...
if __name__ == '__main__':
    ...
    tar_paths_list = list(set(Path(tar_dir).glob('*.tar')) - set(Path(tar_dir).glob('err*.tar')))
    selected_tars_list = list(
        np.array_split(tar_paths_list, SLURM_ARRAY_TASK_COUNT)[SLURM_ARRAY_TASK_ID])
    for tar_path in selected_tars_list:
        ...
        try:
            ...
            resize_imgs(src_root=f'{resized_dir}/{filename_without_ext}', dest_root=f'{resized_dir}/{filename_without_ext}_resized')
